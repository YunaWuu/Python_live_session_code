# PayrollReport01.py
# This program reads employee details from a text file, then processes the payment details and generate the output to a text file. 
# Author: Yanan Wu
# Date: 19/11/2022
...
def readEmployeeInfoFromDataFile(fileName):
    ...
    for line in lines:
        if count == 1:
            employeeId = line.strip()
        elif count == 2:
            name = line.strip()
        elif count == 3:
            position = line.strip()
        elif count == 4:
            pay = float(line.strip())
...
def calculatePayroll():
    ...
    for employee in employees:
        totalPayroll = totalPayroll + employee.pay
...
readEmployeeInfoFromDataFile('PracticalTask2/Employees.txt')
printEmployeeInfo(employees)
calculatePayroll()
printPayrollInfo()
generatePayrollReport('PracticalTask2/PayrollReport.txt')
